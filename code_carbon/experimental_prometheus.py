...
def generate_labels(emissions: dict):
    return [emissions[x] for i,x in enumerate(labelnames)]

class ExperimentalPrometheusOutput(BaseOutput):
    """
    ...
    def add_emission(self, carbon_emission: dict):
        ...
        duration_gauge.labels(*generate_labels(carbon_emission)).set(
            int(carbon_emission["duration"])
        )
        emissions_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["emissions"]
        )
        emissions_rate_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["emissions_rate"]
        )
        cpu_power_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["cpu_power"]
        )
        gpu_power_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["gpu_power"]
        )
        ram_power_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["ram_power"]
        )
        cpu_energy_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["cpu_energy"]
        )
        gpu_energy_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["gpu_energy"]
        )
        ram_energy_gauge.labels(*generate_labels(carbon_emission)).set(
            carbon_emission["ram_energy"]
        )
        energy_consumed_gauge.labels(*generate_labels(carbon_emission)).set(
